...
function run(cmd, cwd) {
    const result = (0, child_process_1.spawnSync)(cmd, { cwd, shell: true, encoding: 'utf-8' });
...
async function postGenerate(config) {
    ...
    const commitResult = run('git commit --no-verify -m "chore: initial scaffold via launch-cli"', projectDir);
    if (commitResult.ok) {
